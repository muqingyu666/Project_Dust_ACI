# -*- coding: utf-8 -*-
# @Author: Muqy
# @Date:   2023-06-26 09:49:03
# @Last Modified by:   Muqy
# @Last Modified time: 2023-11-22 20:10:38
#                  ___====-_  _-====___
#            _--^^^#####//      \\#####^^^--_
#         _-^##########// (    ) \\##########^-_
#        -############//  |\^^/|  \\############-
...
def open_and_reshape_netcdf(path, variables, shape):
    """
    Open a netCDF file and reshape the specified variables to the given shape.
    ...
    variables : list of str
        List of variable names to be reshaped.
    shape : tuple of int
        Desired shape of the variables.

    Returns
    -------
    reshaped_data : list of np.array
        List of the reshaped variables.
    """
...
def flatten_data(data):
    """
    Reshape the input data to a 2D array with shape (-1, 180, 360).
    ...
    -------
    np.array
        Reshaped data with shape (-1, 180, 360).
    """
    data = data[-11:]
...
def get_correlation(PC_data, Cld_data):
    """
    Calculate the Pearson correlation coefficient and p-value between PC1 data and cloud data.
    ...
    -------
    correlation : float
        Pearson correlation coefficient between PC1 data and cloud data.
    p_value : float
        Two-tailed p-value.

    """
    valid_indices = np.isfinite(PC_data.flatten()[:]) & np.isfinite(
        Cld_data.flatten()[:]
    )
...
(
    ...
    PC_path="/RAID01/data/PC_data/1990_2020_best_spatialcorr_var_PC1.nc",
...
(
    ...
    PC_path="/RAID01/data/PC_data/1990_2020_best_corr_var_PC1.nc",
    CERES_Cld_dataset_name="Cldicerad",
)
...
PC_best_corr = PC_best_corr.reshape(31, 12, 28, 180, 360)[
    -11:, :, :, :, :
]
PC_og = PC_og.reshape(31, 12, 28, 180, 360)[-11:, :, :, :, :]

# Reshape the PC data into -1, 180, 360
PC_best_corr_spatial = PC_best_corr_spatial.reshape(-1, 180, 360)
PC_best_corr = PC_best_corr.reshape(-1, 180, 360)
PC_og = PC_og.reshape(-1, 180, 360)

# flatten all cld data to (-1,180,360)
HCF_data = CERES_SSF_HCF.reshape(-1, 180, 360)
...
for level in pressure_levels:
    RH_data, T_data, W_data = reshaped_data[level]
...
for level in pressure_levels:
    ...
    print("Correlation between RH and Cld:")
    corr_RH_Cld, _ = get_correlation_spatial_mean(HCF_data, RH_data)
...
# Calc PC and HCF correlation
corr_PC_HCF_og, _ = get_correlation(HCF_data, PC_og)
corr_PC_HCF_best_corr, _ = get_correlation(HCF_data, PC_best_corr)
corr_PC_HCF_best_corr_spatial, _ = get_correlation(
    HCF_data, PC_best_corr_spatial
)

# Calc PC and HCF correlation
corr_PC_HCF_og, _ = get_correlation_spatial_mean(HCF_data, PC_og)
corr_PC_HCF_best_corr, _ = get_correlation_spatial_mean(
    HCF_data, PC_best_corr
)
corr_PC_HCF_best_corr_spatial, _ = get_correlation_spatial_mean(
    HCF_data, PC_best_corr_spatial
)
...
Corr_Instab_cld_250_300, _ = get_correlation(HCF_data, Instab_250_300)
Corr_Instab_cld_225_250, _ = get_correlation(HCF_data, Instab_225_250)
...
plot_full_hemisphere_self_cmap(    
    data=corr_PC_HCF_og-corr_PC_HCF_best_corr,
    min=-0.2,
    max=0.2,
    var_name="Delta Correlation",
)

plot_full_hemisphere_self_cmap(    
    data=corr_PC_HCF_og-corr_PC_HCF_best_corr_spatial,
    min=-0.2,
    max=0.2,
    var_name="Delta Correlation",
)
...
RelativeH_175 = reshaped_data["175"][0]
Temperature_350 = reshaped_data["350"][1]
Wvelocity_250 = reshaped_data["250"][2]
...
filter_atmos_fit_PC1 = FilterAtmosDataFitPCgap(start, end, gap)
