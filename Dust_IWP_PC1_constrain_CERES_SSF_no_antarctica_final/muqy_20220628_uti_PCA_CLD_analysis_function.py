...
def read_PC1_CERES_clean(PC_path, CERES_Cld_dataset_name):
    """
    Reads the PC1 and CERES data from the specified netCDF files and returns the cleaned data.

    Args:
        PC_path (str): The file path to the netCDF file containing the PC1 data.
        CERES_Cld_dataset_name (str): The name of the CERES dataset to read from the netCDF file.

    Returns:
        tuple: A tuple containing the cleaned PC1 and CERES data.

    Raises:
        FileNotFoundError: If the specified netCDF file(s) cannot be found.
...
def read_PC1_CERES_from_netcdf(CERES_Cld_dataset_name):
    """
...
def np_move_avg(a, n, mode="same"):
    """
    Calculates the n-point moving average of a given array using convolution.

    Args:
        a (numpy.ndarray): The input array.
        n (int): The number of points to use for the moving average.
        mode (str, optional): The mode parameter determines how the edges of the input array are handled.
            Default is "same".

    Returns:
        numpy.ndarray: The n-point moving average of the input array.
    """
...
def plot_global_and_lat_lon_mean(
    ...
    ax.plot(
        lat_mean, np.linspace(-90, 89, len(lat_mean)), color="black"
    )
...
def plot_Cld_no_mean_simple_full_hemisphere_self_cmap(
    ...
    """
    Plots a simple full hemisphere map of cloud data.

    Args:
    - Cld_match_PC_gap: numpy array of cloud data
    - cld_min: minimum value of the cloud data
    - cld_max: maximum value of the cloud data
    - cld_name: name of the cloud data
    - lon: numpy array of longitudes (default: np.linspace(0, 359, 360))
    - lat: numpy array of latitudes (default: np.linspace(-90, 89, 180))
    - cmap_file: path to the colormap file (default: "/RAID01/data/muqy/color/test.txt")
...
def compare_cld_anormally_in_spec_Lat_smoothed(
    ...
    for i, year in enumerate(year_lst):
        axs.plot(
            np.linspace(-180, 179, 360),
            np.nanmean(
                Cld_data_anormally_each_year[i, 110:150, :], axis=0
            ),
            color=color_lst[i],
...
class Filter_data_fit_PC1_gap_plot(object):
    def __init__(self, Cld_data, start, end, gap):
        """
        Initializes an instance of ClassName with the given parameters.

        Args:
            Cld_data (numpy.ndarray): A 3D array of cloud data.
            start (int): The starting index of the time range to analyze.
            end (int): The ending index of the time range to analyze.
            gap (int): The size of the gap to analyze.

        Attributes:
            start (int): The starting index of the time range to analyze.
            end (int): The ending index of the time range to analyze.
            gap (int): The size of the gap to analyze.
            latitude (list): A list of integers representing the latitude indices of the cloud data.
            longitude (list): A list of integers representing the longitude indices of the cloud data.
        """
        self.start = start
        self.end = end
        self.gap = gap
    ...
    def Filter_data_fit_PC1_gap_new_median(self, Cld_data, PC_data):
        """
        Filter data with gap, start and end with giving gap
    ...
    def calc_correlation_PC1_Cld(self, PC_data, Cld_data):
        """
        Calculates the Pearson correlation coefficient between PC1 data and cloud data.

        Parameters
        ----------
        PC_data : array in shape (PC_gap_len, latitude_len, longitude_len)
            PC1 data
        Cld_data : array in shape (PC_gap_len, latitude_len, longitude_len)
            Cloud data

        Returns
        -------
        Correlation : array in shape (180, 360)
            Pearson correlation coefficient between PC1 data and cloud data for each latitude and longitude pair.
        """
    ...
    def Convert_pandas(self, Cld_match_PC_gap):
        """
        Converts a 3D numpy array into a pandas dataframe.

        Args:
        - Cld_match_PC_gap: a 3D numpy array of shape (gap_num, nlat, nlon) containing cloud data.

        Returns:
        - Box: a pandas dataframe of shape (nlat*nlon, gap_num) containing the flattened cloud data.
        """
...
class Filter_data_fit_PC1_gap_plot_IWP_constrain(object):
    def __init__(self, Cld_data, start, end, gap):
        self.start = start
        self.end = end
        self.gap = gap
...
class Filter_data_fit_PC1_IWP_AOD_constrain(object):
    def __init__(self, lat, lon):
        self.latitude = lat
        self.longitude = lon
    ...
    def process_data(
        ...
        # create data mask based on PC, IWP, AOD gap
        mask = (
            (PC_data >= pc_min)
            & (PC_data < pc_max)
            & (IWP_data >= iwp_min)
        ...
        # calculate the mean of cloud fraction, PC, IWP, AOD
        # within the gap
        Cld_match_PC_gap = np.nanmean(
            np.where(mask, Cld_data, np.nan), axis=0
        )
        AOD_match_PC_gap = np.nanmean(
            np.where(mask, AOD_data, np.nan), axis=0
        )
        PC_match_PC_gap = np.nanmean(
            np.where(mask, PC_data, np.nan), axis=0
        )
    ...
    def Filter_data_fit_gap(
        ...
        return Cld_match_PC_gap, AOD_match_PC_gap, PC_match_PC_gap
